...
export default function CitasAgendadasModal({ cita, onClose, onUpdated }: Props) {
  ...
  const estadoColor: Record<string, string> = { pendiente: "#B08968", confirmada: "#2D6A4F", atendida: "#1B4F72", cancelada: "#922B21" };
      ...
      <motion.div initial={{ scale: 0.92, y: 30 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.92, y: 30 }}
        ...
        <div style={{ padding: "1.8rem 2rem 2rem" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
            <button onClick={onClose} style={{ width: 36, height: 36, borderRadius: "50%", background: "#F5EEE6", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><ArrowLeft size={16} /></button>
            <h4 style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, color: "#3A2A1A", margin: 0, fontSize: "1.2rem" }}>Facturacion</h4>
          ...
          {/* Patient */}
          <div style={{ background: "#F5EEE6", borderRadius: 14, padding: "1rem", marginBottom: "0.8rem" }}>
            <p style={{ fontWeight: 600, color: "#3A2A1A", margin: 0 }}>{cita.nombres} {cita.apellidos}</p>
            <p style={{ fontSize: "0.82rem", color: "#6C584C", margin: "0.2rem 0 0" }}>{cita.correo} | {cita.telefono}</p>
            <p style={{ fontSize: "0.82rem", color: "#6C584C", margin: "0.15rem 0 0" }}>{String(cita.fecha).slice(0, 10)} - {cita.hora}</p>
          </div>
          ...
          {cita.estado !== "cancelada" && cita.estado !== "atendida" && (
            <div style={{ marginBottom: "1.5rem" }}>
              <h5 style={{ fontWeight: 600, color: "#3A2A1A", marginBottom: "1rem", display: "flex", alignItems: "center", gap: 6, fontSize: "1rem" }}>
                <Calculator size={16} color="#B08968" /> Registro de pago
              </h5>
              ...
              {montoNum > 0 && pagoNum > 0 && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} style={{ overflow: "hidden" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.82rem", marginBottom: 6 }}>
                    <span style={{ color: "#6C584C" }}>Pagado: <strong>${pagadoTotal.toLocaleString("es-CO")}</strong></span>
                    {restante > 0 && <span style={{ color: "#B08968", fontWeight: 600 }}>Restante: ${restante.toLocaleString("es-CO")}</span>}
                    {devolucion > 0 && <span style={{ color: "#2E7D32", fontWeight: 600 }}>Devolucion: ${devolucion.toLocaleString("es-CO")}</span>}
                  </div>
                  <div style={{ height: 8, borderRadius: 4, background: "#E9DED2", overflow: "hidden" }}>
        ...
        {showConfirm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.85, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.85, y: 20 }}
              ...
              <h4 style={{ fontWeight: 700, color: "#3A2A1A", marginBottom: "0.5rem" }}>Confirmar cita?</h4>
              <p style={{ fontSize: "0.85rem", color: "#6C584C", marginBottom: "1.5rem" }}>Cita de <strong>{cita.nombres}</strong> para <strong>{cita.procedimiento}</strong></p>
              <div style={{ display: "flex", gap: "0.8rem", justifyContent: "center" }}>
                <button onClick={() => setShowConfirm(false)} style={{ padding: "0.6rem 1.3rem", borderRadius: 100, border: "1px solid #E9DED2", background: "transparent", color: "#6C584C", fontWeight: 600, cursor: "pointer" }}>Cancelar</button>
