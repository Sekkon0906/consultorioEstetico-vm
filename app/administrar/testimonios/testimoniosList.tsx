          ...
          {comentarios.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.9rem" }}>
              {comentarios.map(function(c) {
                return (
                  <div key={c.id} style={{ background: "var(--surface)", borderRadius: 18, border: "1px solid " + (c.aprobado ? "#A0D8A8" : "#F0E0A0"), padding: "1.1rem 1.4rem", display: "flex", alignItems: "center", gap: "1.1rem" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", gap: 2, marginTop: 4 }}>
                        {[1,2,3,4,5].map(function(i) { return <span key={i} style={{ color: i <= c.puntuacion ? "#C0A080" : "#ddd", fontSize: "0.95rem" }}>â˜…</span>; })}
                      </div>
                    </div>
                    <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                      <button onClick={function() { toggleAprobado(c.id, c.aprobado); }} style={{ padding: "0.5rem 1rem", borderRadius: 100, background: c.aprobado ? "#FFF3E6" : "#D8F3DC", color: c.aprobado ? "var(--brand)" : "#2E7D32", border: "none", fontSize: "0.85rem", fontWeight: 600, cursor: "pointer" }}>{c.aprobado ? "Ocultar" : "Aprobar"}</button>
          ...
          {list.length === 0 ? <p style={{ textAlign: "center", color: "var(--text-soft)", padding: "2rem 0" }}>No hay testimonios</p> : (
            <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
              {list.map(function(t, i) {
                return (
                  <motion.div key={t.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.02 }} style={{ background: "var(--surface)", borderRadius: 18, border: "1px solid var(--border)", padding: "1.1rem 1.4rem", display: "flex", alignItems: "center", gap: "1.1rem", opacity: t.activo ? 1 : 0.5 }}>
                    {t.thumb ? <img src={t.thumb} alt="" style={{ width: 72, height: 72, borderRadius: 14, objectFit: "cover", flexShrink: 0 }} /> : <div style={{ width: 72, height: 72, borderRadius: 14, background: "var(--border)", flexShrink: 0 }} />}
                    <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: "flex", gap: 7, flexShrink: 0 }}>
                      <IBtn icon={t.activo ? <Eye size={18} /> : <EyeOff size={18} />} bg={t.activo ? "#D8F3DC" : "var(--surface-soft)"} onClick={function() { toggle(t, "activo"); }} />
